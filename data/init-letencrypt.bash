...
for domain in ${!domains[*]}; do
  ...
  if [ ! -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    docker exec certbot sh -c "openssl req -x509 -nodes -newkey rsa:1024 -days 1 -keyout '$path/privkey.pem' -out '$path/fullchain.pem' -subj '/CN=localhost'"
    #docker compose run --rm --volume ["certbot_config:/etc/letsencrypt","certbot_www:/var/www/certbot"] \
    #--entrypoint "openssl req -x509 -nodes -newkey rsa:1024 \
    #-days 1 -keyout '$path/privkey.pem' -out '$path/fullchain.pem' -subj '/CN=localhost'" certbot
  fi

done
echo "### Starting nginx ..."
docker compose up -d router && docker compose restart router
...
for domain in ${!domains[*]}; do
  ...
  if [ -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    docker exec certbot sh -c "certbot certonly --webroot -w /var/www/certbot --cert-name $domain_name $domain_args \
    $staging_arg $email_arg --rsa-key-size $rsa_key_size --agree-tos --force-renewal --non-interactive"
    #docker compose run --rm --volume [certbot_config:/etc/letsencrypt,certbot_www:/var/www/certbot] \
    #--entrypoint "certbot certonly --webroot -w /var/www/certbot --cert-name $domain_name $domain_args \
